...
def call_gemini(user_input: str) -> dict:
    ...
    Returns:
        dict with keys: symptoms (list[str]), condition (str), error (str|None)
    """
    ...
    try:
        ...
        return {"symptoms": [], "condition": "", "error": "Invalid JSON from Gemini."}

    except Exception as exc:
        logger.error("Gemini API call failed: %s", exc)
        return {"symptoms": [], "condition": "", "error": str(exc)}
